...
const ValidateTransaction = {
    validateStandaloneTx: (transaction, transactionDTO) => {
        deepEqual(transaction.transactionInfo.height, BigInt(transactionDTO.meta.height));
        ...
        deepEqual(transaction.maxFee, BigInt(transactionDTO.transaction.maxFee));
    ...
    validateAggregateTx: (aggregateTransaction, aggregateTransactionDTO) => {
        deepEqual(aggregateTransaction.transactionInfo.height, BigInt(aggregateTransactionDTO.meta.height));
        ...
        deepEqual(aggregateTransaction.maxFee, BigInt(aggregateTransactionDTO.transaction.maxFee));
    ...
    validateMosaicCreationTx: (mosaicDefinitionTransaction, mosaicDefinitionTransactionDTO) => {
        ...
        deepEqual(mosaicDefinitionTransaction.duration, BigInt(mosaicDefinitionTransactionDTO.transaction.duration));
    ...
    validateMosaicSupplyChangeTx: (mosaicSupplyChangeTransaction, mosaicSupplyChangeTransactionDTO) => {
        ...
        deepEqual(mosaicSupplyChangeTransaction.delta, BigInt(mosaicSupplyChangeTransactionDTO.transaction.delta));
    },
    ...
    validateNamespaceCreationTx: (registerNamespaceTransaction, registerNamespaceTransactionDTO) => {
        ...
        if (registerNamespaceTransaction.registrationType === 0) {
            deepEqual(registerNamespaceTransaction.duration, BigInt(registerNamespaceTransactionDTO.transaction.duration));
        } else {
            deepEqual(
