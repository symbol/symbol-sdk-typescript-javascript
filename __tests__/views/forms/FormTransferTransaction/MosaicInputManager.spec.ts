...
describe('MosaicInputManager ==>', () => {
  describe('initialize should return an instantiated object', () => {
    expect(MosaicInputsManager.initialize(mockMosaics)).toBeInstanceOf(MosaicInputsManager)
  })

  describe('hasFreeSlots should return true after initialization with mosaics provided', () => {
    const mosaicInputsManager = MosaicInputsManager.initialize(mockMosaics)
    expect(mosaicInputsManager.hasFreeSlots()).toBeTruthy()
  })

  describe('hasFreeSlots should return false after initialization with empty array', () => {
    const mosaicInputsManager = MosaicInputsManager.initialize([])
    expect(mosaicInputsManager.hasFreeSlots()).toBeFalsy()
  })

  describe('getMosaicsBySlot should return all mosaics after initialization with mosaics provided, for any slot', () => {
    const mosaicInputsManager = MosaicInputsManager.initialize(mockMosaics)
  ...
  describe('getMosaicsBySlot should return an empty array after after initialization with empty array, for any slot', () => {
    const mosaicInputsManager = MosaicInputsManager.initialize([])
  ...
  describe('setSlot should throw if a an unknown id is provided', () => {
    const mosaicInputsManager = MosaicInputsManager.initialize([])
    expect(() => mosaicInputsManager.setSlot('wrongHexId', 1)).toThrowError()
  })
  
  describe('setSlot should throw if a mosaic is already affected to the slot', () => {
    const mosaicInputsManager = MosaicInputsManager.initialize(mockMosaics)
    mosaicInputsManager.setSlot(mockMosaicHexIds[1], 2)
    expect(() => mosaicInputsManager.setSlot(mockMosaicHexIds[1], 1)).toThrowError()
  })

  describe('getMosaicsBySlot should expected mosaics after affectations', () => {
    const mosaicInputsManager = MosaicInputsManager.initialize(mockMosaics)
    mosaicInputsManager.setSlot(mockMosaicHexIds[1], 2)
