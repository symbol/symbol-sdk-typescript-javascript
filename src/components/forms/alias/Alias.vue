<template>
  <div class="aliasTable">
    <Modal
      v-model="show" :transfer="false" class="alias_bind_dialog"
      @on-cancel="show = false"
    >
      <form action="submit" onsubmit="event.preventDefault()" @keyup.enter="submit">
        <div slot="header" class="bindHeader">
          <span v-if="bind" class="title">{{ $t('bind') }}</span>
          <span v-else class="title">{{ $t('unbind') }}</span>
        </div>
        <DisabledForms />
        ...
        <div v-if="bind && fromNamespace && bindType === BindTypes.ADDRESS" class="input_content">
          ...
          <div class="input_area">
            <ErrorTooltip field-name="address" class="flex-8" placement-override="top">
              <input
                v-model="target"
        ...
        <div v-if="bind && fromNamespace && bindType === BindTypes.MOSAIC" class="input_content">
          ...
          <ErrorTooltip field-name="mosaicId" class="flex-8" placement-override="top">
            <Select
              v-model="target"
              v-validate="'required'"
        ...
        <div v-if="bind && !fromNamespace" class="input_content">
          ...
          <ErrorTooltip field-name="namespace" class="flex-8" placement-override="top">
            <Select
              v-model="alias"
              v-validate="'required'"
              class="fee-select input_select"
