...
import {AppInfo, AppWallet, AppNamespace, DefaultFee, MosaicNamespaceStatusType, BindTypes} from '@/core/model'
...
export class AliasTs extends Vue {
  ...
  get linkableMosaics(): string[] {
    ...
    return availableToBeLinked
      .filter(
        (item) => currentHeight < item.expirationHeight || item.expirationHeight === MosaicNamespaceStatusType.FOREVER,
      )
      .map(({hex}) => hex)
  }
