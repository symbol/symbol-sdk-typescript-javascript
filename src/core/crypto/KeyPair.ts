...
const catapult_crypto = (function() {
    function clamp(d) {
        d[0] &= 248;
        d[31] &= 127;
        d[31] |= 64;
 ...
 */
export const createKeyPairFromPrivateKeyString = (privateKeyString) => {
    ...
    const publicKey = catapult_crypto.extractPublicKey(privateKey, catapult_hash.func);
    return {
        privateKey,
        publicKey,
    };
 ...
 */
export const sign = (keyPair, data) => {
    return catapult_crypto.sign(data, keyPair.publicKey, keyPair.privateKey, catapult_hash.createHasher());
};
 ...
 */
export const verify = (publicKey, data, signature) => {
    return catapult_crypto.verify(publicKey, data, signature, catapult_hash.createHasher());
};
 ...
 */
export const deriveSharedKey = (keyPair, publicKey, salt) => {
    if (Key_Size !== salt.length) {
        throw Error(`salt has unexpected size: ${salt.length}`);
    }

    return catapult_crypto.deriveSharedKey(salt, keyPair.privateKey, publicKey, catapult_hash.func);
};

export const hash = catapult_hash;
