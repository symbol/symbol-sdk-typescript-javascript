...
export class MultisigHttp extends Http implements MultisigRepository {
    ...
    public getMultisigAccountInfo(address: Address): Observable<MultisigAccountInfo> {
        return this.networkTypeObservable.pipe(
            mergeMap((networkType) =>
                observableFrom(this.multisigRoutesApi.getAccountMultisig(address.plain())).pipe(
                    map(
                        ({ body }) =>
                            new MultisigAccountInfo(
                                ...
                                body.multisig.cosignatoryPublicKeys.map((cosigner) =>
                                    PublicAccount.createFromPublicKey(cosigner, networkType),
                                ),
                                body.multisig.multisigPublicKeys.map((multisigAccount) =>
                                    PublicAccount.createFromPublicKey(multisigAccount, networkType),
                                ),
                            ),
                    ),
    ...
    public getMultisigAccountGraphInfo(address: Address): Observable<MultisigAccountGraphInfo> {
        return this.networkTypeObservable.pipe(
            mergeMap((networkType) =>
                observableFrom(this.multisigRoutesApi.getAccountMultisigGraph(address.plain())).pipe(
                    map(({ body }) => {
                            ...
                            multisigAccounts.set(
                                ...
                                multisigAccountGraphInfoDTO.multisigEntries.map((multisigAccountInfoDTO) => {
                                    return new MultisigAccountInfo(
                                        ...
                                        multisigAccountInfoDTO.multisig.cosignatoryPublicKeys.map((cosigner) =>
                                            PublicAccount.createFromPublicKey(cosigner, networkType),
                                        ),
                                        multisigAccountInfoDTO.multisig.multisigPublicKeys.map((multisigAccountDTO) =>
