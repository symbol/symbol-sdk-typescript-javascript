...
import { InnerTransactionBigInt } from './InnerTransactionBigInt';

/**
 * An abstract transaction class that serves as the base class of all NEM transactions.
 */
export abstract class TransactionBigInt {
     ...
     * @var {number}
     */
    public static readonly Type_Index: number = TransactionBigInt.Header_Size + 2;
     ...
     * @var {number}
     */
    public static readonly Body_Index: number = TransactionBigInt.Header_Size + 1 + 1 + 2 + 8 + 8;
    ...
    public static createTransactionHash(transactionPayload: string, generationHashBuffer: number[], networkType: NetworkType): string {
        ...
        const typeIdx: number = TransactionBigInt.Type_Index;
        ...
        let transactionBody: Uint8Array = transactionBytes.slice(TransactionBigInt.Header_Size);

        // in case of aggregate transactions, we hash only the merkle transaction hash.
        if (isAggregateTransaction) {
            transactionBody = transactionBytes.slice(TransactionBigInt.Header_Size, TransactionBigInt.Body_Index + 32);
        }
     ...
     */
    abstract resolveAliases(statement?: Statement, aggregateTransactionIndex?: number): TransactionBigInt;
     ...
     */
    public setMaxFee(feeMultiplier: number): TransactionBigInt {
    ...
    public signWith(account: Account, generationHash: string): SignedTransaction {
        ...
        return new SignedTransaction(
            payload,
            TransactionBigInt.createTransactionHash(payload, generationHashBytes, account.networkType),
            account.publicKey,
            this.type,
            this.networkType);
    }
     ...
     */
    public toAggregate(signer: PublicAccount): InnerTransactionBigInt {
        if (this.type === TransactionType.AGGREGATE_BONDED || this.type === TransactionType.AGGREGATE_COMPLETE) {
     ...
     */
    public reapplyGiven(deadline: Deadline = Deadline.create()): TransactionBigInt {
